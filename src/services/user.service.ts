...
import { TUser, TUserPassword } from "../util/types"
...
export class UserService {
  ...
  async changePassword(_id: string, body: TUserPassword) {
    try {
      const user = await this.getUser(body.user_id)
      ...
      if (await comparePassword(oldPassword, user.password)) {
        ...
        user.save()
        return user
      } else throw new ApiError(false, 400, "Old password is incorrect")
    } catch (e: any) {
